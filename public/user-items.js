document.addEventListener('DOMContentLoaded', function() {
    ...
    async function deleteItem(itemId) {
        if (confirm('Are you sure you want to delete this item?')) {
            const response = await fetch(`/item/delete/${itemId}`, { method: 'POST' });
            if (response.ok) {
                loadUserItems();
            } else {
    ...
    async function returnItem(itemId) {
        if (confirm('Are you sure this is not the owner? This will make the item available for claims again.')) {
            const response = await fetch(`/claim/return/${itemId}`, { method: 'POST' });
            if (response.ok) {
                loadUserItems();
            } else {
