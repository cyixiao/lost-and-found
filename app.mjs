...
app.post('/claim', async (req, res) => {
    ...
    if (!item_id || !claimer_id) {
        return res.status(400).send("Missing required information: item_id and claimer_id are both required.");
    }
    try {
        const user = await db.get("SELECT contact_info FROM Users WHERE user_id = ?", [claimer_id]);
        if (!user) {
            return res.status(404).send("User not found. Invalid claimer_id.");
        }
        await db.run("INSERT INTO Claims (item_id, claimer_id, contact_info, status) VALUES (?, ?, ?, 0)", [item_id, claimer_id, user.contact_info]);
        await db.run("UPDATE Items SET status = 1, claimer_id = ? WHERE item_id = ?", [claimer_id, item_id]);
...
app.post('/claim/end/:item_id', async (req, res) => {
    ...
    try {
        const claim = await db.get("SELECT claim_id FROM Claims WHERE item_id = ? AND status = 0", [item_id]);
        if (!claim) {
            return res.status(404).send("No active claim found for this item.");
        }
        await db.run("UPDATE Claims SET status = 1 WHERE claim_id = ?", [claim.claim_id]);
...
app.post('/claim/return/:item_id', async (req, res) => {
    ...
    try {
        ...
        await db.run("UPDATE Claims SET status = 1 WHERE claim_id = ?", [claim.claim_id]);
        await db.run("UPDATE Items SET status = 0, claimer_id = NULL WHERE item_id = ?", [item_id]);
        res.send("Claim ended, item status reset, and claimer removed successfully.");
    } catch (error) {
        res.status(500).send("Internal Server Error: " + error.message);
    }
});
...
app.get('/claim/info/:item_id', async (req, res) => {
    ...
    try {
        const claimInfo = await db.get(`
            ...
            WHERE c.item_id = ? AND c.status = 0`, [item_id]);
        if (!claimInfo) {
            return res.status(404).send("No active claim found for this item or claimer does not exist.");
        }
        res.json({
